...
const fetchProducts = async () => {
  ...
  try {
    ...
    products.value = response.data.data || response.data // Sesuaikan struktur JSON Phalcon
  } catch (error) {
    ...
    if (products.value.length === 0) {
      products.value = [
        { id: 1, name: 'Konsultasi Dokter Umum', type: 'Jasa', price: 100000 },
        { id: 2, name: 'Obat Paracetamol', type: 'Barang', price: 15000 }
      ]
    }
  } finally {
...
const openEditModal = (item: any) => {
  ...
  formData.value = { ...item } // Copy data ke dalam form
  showModal.value = true
}
...
const saveProduct = async () => {
  try {
    if (isEditMode.value) {
      // PROSES UPDATE (PUT)
      // await axios.put(`http://localhost:8000/healthcare-items/${formData.value.id}`, formData.value)
      
      // Simulasi update di layar (Hapus baris ini jika API sudah jalan)
      ...
      // PROSES CREATE (POST)
      // await axios.post('http://localhost:8000/healthcare-items', formData.value)
      
      // Simulasi tambah di layar (Hapus baris ini jika API sudah jalan)
      products.value.push({ ...formData.value, id: Date.now() }) 
      alert('Data berhasil ditambahkan!')
    }
    
    showModal.value = false
    // fetchProducts() // Refresh data dari server (Aktifkan jika API sudah jalan)
  } catch (error) {
    console.error(error)
...
const deleteProduct = async (id: number) => {
  ...
  try {
    // await axios.delete(`http://localhost:8000/healthcare-items/${id}`)
    
    // Simulasi hapus di layar (Hapus baris ini jika API sudah jalan)
